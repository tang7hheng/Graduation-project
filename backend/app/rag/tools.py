...
class SearchProductsInput(BaseModel):
    ...
    top_k: int = Field(default=5, ge=1, le=10, description="返回商品数量,默认5,最多10")
...
def search_products_by_keyword(keyword: str, top_k: int = 5) -> str:
    ...
    try:
        ...
        # Use a lower k internally to have room for threshold filtering
        results = vs.similarity_search_with_score(keyword, k=max(top_k * 2, 10))
    except Exception as e:
    ...
    # Use the shared threshold from settings for consistency across the app
    ...
    for doc, distance in results:
        ...
        # Hard threshold filter: reject low-quality matches
