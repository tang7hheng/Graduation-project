...
onMounted(async () => {
  await orders.fetchList(chat.currentSessionId || undefined)
})

function statusInfo(status: string) {
  switch (status) {
    case 'paid':
      return { type: 'success' as const, label: '已支付' }
    case 'cancelled':
      return { type: 'info' as const, label: '已取消' }
    default:
      return { type: 'warning' as const, label: '待支付' }
  }
}
